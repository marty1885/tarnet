...
mod tests {
    ...
    #[test]
    fn label_store_migration_from_petnames() {
        use crate::state::StateDb;
        use rusqlite::Connection;

        let path = std::env::temp_dir().join(format!(
            "tarnet-migrate-{}.sqlite3",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
                .as_nanos()
        ...
        // Create a legacy database with the petnames table.
        {
            let conn = Connection::open(&path).unwrap();
            conn.execute_batch(
                "CREATE TABLE petnames (
                     name TEXT PRIMARY KEY,
                     zone_pubkey BLOB NOT NULL
                 );",
            )
            .unwrap();
            conn.execute(
                "INSERT INTO petnames(name, zone_pubkey) VALUES('alice', ?1)",
                rusqlite::params![&[0x42u8; 32][..]],
            )
            .unwrap();
            conn.execute(
                "INSERT INTO petnames(name, zone_pubkey) VALUES('bob', ?1)",
                rusqlite::params![&[0x43u8; 32][..]],
            )
            .unwrap();
        }

        // Open with StateDb — should auto-migrate.
        let db = StateDb::open(&path).unwrap();

        // petnames table should be gone.
        let conn = db.lock_for_test();
        let has_petnames: i64 = conn
            .query_row(
            ...
            .unwrap();
        assert_eq!(has_petnames, 0, "petnames table should be dropped after migration");
        drop(conn);

        // Labels should exist with Zone records and publish=false.
        let list = db.label_list().unwrap();
        assert_eq!(list.len(), 2);
        assert_eq!(list[0].0, "alice");
        assert_eq!(list[1].0, "bob");
        assert!(!list[0].2); // publish=false
        assert!(!list[1].2);

        // Each should have one record blob (tag 0x01 = Zone + 32 bytes).
        assert_eq!(list[0].1.len(), 1);
        assert_eq!(list[1].1.len(), 1);
        let (rec, _) = tns_record_from_bytes(&list[0].1[0]).unwrap();
        assert!(matches!(rec, TnsRecord::Zone(_)));
